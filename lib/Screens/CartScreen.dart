...
class _CartScreenState extends State<CartScreen> {
          ...
          if (productInfo.status) {
            setState(() {
              cartItems.add(CartInfo(
                  products: productInfo.data['data'].length > 0
                      ? Products.fromJson(productInfo.data['data'][0])
                      : Products(),
                              ...
                              SizedBox(
                                  ...
                                  child: FlatButton(
                                      child: item.isDeleting
                                          ...
                                          : Icon(Icons.remove,
                                              color: Colors.white),
                                      onPressed: item.isDeleting
                                          ? null
                                          : () => _removeQuantity(item),
                                      color: Myapp.primaryColor,
  ...
  void _addToCart(CartInfo item) async {
    ...
    if (responseData.status) {
      ...
      if (cartId.status) {
        ResponseData updateQuantity = await ApiService.updateCartItemQuantity(
            token: responseData.token,
            itemId: item.cartItem.itemId.toString(),
            qty: (item.cartItem.qty + 1).toString(),
            quoteId: cartId.data.toString());
  ...
  _removeQuantity(CartInfo item) async {
    ...
    if (item.cartItem.qty != 1) {
      ...
      ResponseData deleteCartItem = await ApiService.removeItemFromCart(
          itemId: item.cartItem.itemId.toString(), token: responseData.token);
